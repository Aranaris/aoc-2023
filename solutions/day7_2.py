...
# with open('../test-inputs/day7testinput.txt') as file:
	...
	for i in range(len(hands)):
		...
		for card in hand:
			if card == 'J':
				j_count += 1
			elif card not in hand_dict['cards']:
				hand_dict['cards'][card] = 1
			else:
				hand_dict['cards'][card] += 1
		if hand_dict['cards']:
			max_count = max(hand_dict['cards'].values()) + j_count
		else:
			max_count = 5
		...
		if max_count >= 5:
			...
			hand_dict['type'] = 'Four'
		elif max_count == 3 and 2 in hand_dict['cards'].values() and j_count == 0:
			hand_dict['type'] = 'Full House'
		elif max_count == 3 and j_count > 0:
